...
public class TupleUtils
{
  ...
  case AttrType.attrDesc: 
    try {
      t1_desc = t1.getDescFld(t1_fld_no);
      t2_desc = t2.getDescFld(t2_fld_no);
    }catch (FieldNumberOutOfBoundException e){
      throw new TupleUtilsException(e, "FieldNumberOutOfBoundException is caught by TupleUtils.java");
    }
    ...
    double sum = 0;
    for(int i = 0;i < 5; ++i){
      sum += Math.pow( ( t1_desc.get(i) - t2_desc.get(i) ), 2);
    }
     ...
     TupleUtilsException
    {
      ...
      switch (fldType.attrType){
        case AttrType.attrDesc: 
          ...
          for(int i = 0;i < 5; ++i){
            if(target.get(i) != -1) isdefault = false;
          }
          if(isdefault){
            double dist = t1_desc.distance(t2_desc);
            if(dist <= distance) return 0;
            else return 1;
          }else{
            double dist1 = t1_desc.distance(target);
            double dist2 = t2_desc.distance(target);
  ...
  public static boolean Equal(Tuple t1, Tuple t2, AttrType types[], int len, double distance, Descriptor target)
    throws IOException,UnknowAttrType,TupleUtilsException
    {
      ...
      for (i = 1; i <= len; i++){
        if(isdefault){
          if (CompareTupleWithTuple(types[i-1], t1, i, t2, i, distance, target) == 0) return true;
          else return false;
        }else{
          if (CompareTupleWithTuple(types[i-1], t1, i, t2, i, distance, target) == 0) return true;
          else return false;
