...
public class Sort extends Iterator implements GlobalConst
{
	   ...
	   Exception
  {
    ...
    while (true) {
      ...
      if(sortFldType.attrType!=AttrType.attrDesc)
		  ...
		  double distance=target.distance(cur_node.tuple.getDescFld(_sort_fld));	//get the distance between current node's descriptor and target
  ...
  private void MIN_VAL(Tuple lastElem, AttrType sortFldType) 
	...
	case AttrType.attrDesc:
	  lastElem.setDescFld(_sort_fld, target);	//Min value is target
    default:
  ...
  private void MAX_VAL(Tuple lastElem, AttrType sortFldType) 
	...
	case AttrType.attrDesc:
	  ...
	  d.set(target.get(0)<0?(-1):1*Integer.MIN_VALUE, 
		target.get(1)<0?(-1):1*Integer.MIN_VALUE, 
		target.get(2)<0?(-1):1*Integer.MIN_VALUE, 
		target.get(3)<0?(-1):1*Integer.MIN_VALUE, 
		target.get(4)<0?(-1):1*Integer.MIN_VALUE);
	  lastElem.setDescFld(_sort_fld, d);	/*d is the max value of descriptor*/
    default:
      // don't know how to handle attrSymbol, attrNull
      //System.err.println("error in sort.java");
      throw new UnknowAttrType("Sort.java: don't know how to handle attrSymbol, attrNull");
 ...
 /**
  * Reload Sort() to add target parameter
  */
      public Sort(AttrType[] in,         
	      short      len_in,             
	      short[]    str_sizes,
