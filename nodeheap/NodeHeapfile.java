...
public class NodeHeapfile implements Filetype,  GlobalConst {
  ...
  */
  private boolean  _findDataPage( NID nid,
  ...
  public int getNodeCnt() 
      ...
      while(currentDirPageId.pid != INVALID_PAGE)
	{
	   ...
	   for (nid = currentDirPage.firstRecord();
	        nid != null;	// nid==NULL means no more record
   ...
   * @param newnode: the new content of the record
   *
   * @exception InvalidSlotNumberException invalid slot number
   ...
   * @exception Exception other exception
   *
   * @return a Node. if Node==null, no more tuple
   */
